...
The background chapter establishes the theoretical role of exact lattice gauge
invariance and the observables used to monitor the continuum limit. The open
question for the present work is how a controlled violation of this symmetry
enters the scaling analysis. The deformation must be specified at the level of
the lattice action or update rule, and its effect must then be tested through
dimensionless observables that can be compared across lattice spacings.

\section{Definition of the Gauge-Breaking Deformation}
Define the modified lattice action or update rule used in the simulations.
Specify the breaking parameters, for example $\eps_1$ and $\eps_2$, and explain
which gauge transformations are no longer exact symmetries.

\section{Symmetry and Scaling Expectations}
\subsection{Relevant, Marginal, and Irrelevant Breaking Terms}
Classify the deformation in renormalization-group language and state which
scaling behavior would be expected for each case.

\subsection{Possible Restoration of Gauge Invariance}
Explain under which conditions gauge-invariant observables can approach the
same continuum limit even when gauge invariance is broken at finite lattice
spacing.

\subsection{Expected Dependence on Breaking Strength and Lattice Spacing}
State the expected dependence on $\eps_1$, $\eps_2$, and $\latspacing$, and use
this expectation to motivate the fit forms introduced in the methodology
chapter.
