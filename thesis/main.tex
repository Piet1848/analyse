...
and systematic errors relevant for the analysis.
...
In this thesis the precise normalization convention used in the simulation must
be stated explicitly in the numerical setup. Additive constants in the action do
not affect expectation values, because they cancel between numerator and
denominator in normalized path-integral averages and do not change Metropolis
...
some perimeter and normalization contributions. A common definition is
\begin{equation}
  \chi(R,T)
  =
  -
...
Define the extrapolation variable, for example $(\latspacing/\rzero)^2$, $1/\beta$, or another measured scale proxy.

\section{Fixed-Physics Trajectories}
Explain how ensembles at different $\beta$ and $\eps$ are compared. State whether $\eps$ is held fixed in bare units, scaled with $\latspacing$, or tuned along the trajectory.

\section{Fit Ansatz}
\subsection{Gauge-Invariant Baseline}
\subsection{Linear and Quadratic Lattice-Spacing Corrections}
\subsection{Combined Fits in Lattice Spacing and Breaking Strength}

\section{Uncertainty Budget}
\subsection{Statistical Errors}
\subsection{Fit-Window Systematics}
\subsection{Finite-Volume Effects}
\subsection{Continuum-Fit Systematics}
